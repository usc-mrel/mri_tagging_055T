function [img,gmap,snr,snr_pseudo,gmap_pseudo,noise_psf_pseudo] = ismrm_cartesian_SENSE(inp,csm,acc_factor,replicas,samp_mat)
%
...
if (nargout > 3),
    image_formation_func = @(x) sum(ismrm_transform_kspace_to_image(sqrt(acc_factor)*x .* repmat(samp_mat,[1 1 size(csm,3)]),[1,2]).*unmix_sense,3);
